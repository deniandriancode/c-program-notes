...
void create_box (struct WIN *, bool);
...
void
init_win_params (struct WIN *p_win)
{
        p_win->height = 3;
        p_win->width = 10;
...
void
print_win_params (struct WIN *p_win)
{
        #ifdef _DEBUG
            ...
            bool flag)
{
        ...
        if (flag == TRUE) {
                ...
                mvvline (y+1, x, p_win->border.rs, h - 1);
        }
        else {
                for (j = y; j <= y; ++j) {
                        for (i = x; i <= x; ++i) {
